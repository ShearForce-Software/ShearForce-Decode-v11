...
- [ ] INTAKE-1: Add logic for the auto intake to be on if the lifter is in the UP position
  * this ensures that if a ball is stuck on the intake it gets pushed through
- [ ] INTAKE-2: Test the new break beams, ensure auto logic is working right
  * should be able to just drive around and auto intake balls and shoot them when you see green middle
- [ ] INTAKE-3: Add auto intake logic to second thread in redFarAuto, comment out other intake commands, TEST
  * be sure to put inside a check that run has been started (don't want auto intake running during init)
- [ ] INTAKE-4: if redFarAuto changes work, then implement and test on the other auto routines
