...
- [ ] AUTO: change shoot spin up to be while < -200 set to be -100, while < -100 set to be -50 -- RETEST
- [ ] AUTO: Add 2? second timeout on shots
- Tune built-in PIDF using dashboard to achieve faster recovery without overshoots
  * [ ] Run Manual, find velocity and power values for small triangle position, RECORD in comment in SetShooterPIDFCoefficients()
  * [ ] Run Manual, find velocity and power values for AUTO Close position, RECORD in comment in SetShooterPIDFCoefficients()
  * [ ] Run manual, find current PIDF values from telemetry, RECORD in comment in SetShooterPIDFCoefficients()
  * [ ] Change shooterF to be 80% of velocity value for small triangle shot above, set shooterPIDF_Enabled to true
  * [ ] Using Dashboard, and running manual, increase **shooterP** slowly until motor velocity starts to oscillate around target velocity
