...
- [ ] AUTO: Test: Change Close Autos to initialize shooter motor to correct speed instead of max like it is currently doing
...
- [ ] Change Target positions to be back corner of goal instead of front center
- [ ] Create a distance calculator that uses the roadrunner position instead of the apriltag
- [ ] Test and get working the ability to update the roadrunner position based on the apriltag, needs to only do an update if off by xxx inches and velocity = 0
- [ ] Update CalculateOptimumShooterRPM() to use dynamic range logic using a % of distance between measurements, rather than flat step functions
  ** Example:
```
