...
public class RedBoardAlternate_Gertrude extends LinearOpMode {
    ...
    public void runOpMode(){
        ...
        Actions.runBlocking(
                drive.actionBuilder(drive.localizer.getPose())
                        .strafeToLinearHeading(new Vector2d(stackX-2,drive.localizer.getPose().position.y - control.distanceCorrectionLR_HL), Math.toRadians(180))
                        .build()
        );
        ...
        if (control.autoPosition == 2) {
            ...
            /*Actions.runBlocking(
                    drive.actionBuilder(drive.localizer.getPose())
                            .strafeToLinearHeading(new Vector2d(stackX, drive.localizer.getPose().position.y - control.distanceCorrectionLR_HL), Math.toRadians(180))
                            .build()
            );
            drive.updatePoseEstimate();*/
